...
public:
	...
	inline
	const Simd<float, 4>& vv_v(uint32_t n, uint32_t i, uint32_t j, uint32_t vk) const
	{
		return GetOprVec(OP_TYPE::VV, n, i, j, vk);
	}

	inline
	const Simd<float, 4>& vi_v(uint32_t n, uint32_t i, uint32_t j, uint32_t vk) const
	{
		return GetOprVec(OP_TYPE::VI, n, i, j, vk);
	}

	inline
	const Simd<float, 4>& iv_v(uint32_t n, uint32_t i, uint32_t j, uint32_t vk) const
	{
		return GetOprVec(OP_TYPE::IV, n, i, j, vk);
	}

	inline
	const Simd<float, 4>& ii_v(uint32_t n, uint32_t i, uint32_t j, uint32_t vk) const
	{
...
private:
	...
	float GetOpr(uint32_t type, uint32_t n, uint32_t i, uint32_t j, uint32_t k) const
	{
		...
		if (m_compOprArr[type] != NULL) {
			...
			ArrayLib::ArrayNIJK<Simd<float, 4>>& oprArr = *m_rawOprArr[type];
	...
	inline
	const Simd<float, 4>& GetOprVec(uint32_t type, uint32_t n, uint32_t i, uint32_t j, uint32_t vk) const
	{
		ArrayLib::ArrayNIJK<uint32_t, uint32_t, 4>& index = *m_index;
	...
	std::array<
		...
		ArrayLib::ArrayNIJK<Simd<float, 4>>*,
		4
