...
void Engine_Tiling::Init()
{
	...
	// TODO: first touch
	// TODO: modify ArrayLib to include a "no memset()" option for first touch
	uint32_t numVectors = ceil((double) numLines[2] / 4.0);

	m_volt_v = new ArrayNIJK<Simd<float, 4>, size_t>(
		"volt_v", {numLines[0], numLines[1], numVectors}
	);
	//ParaExec::firstTouch(*m_volt_v, Op->m_perNodePlan);

	m_curr_v = new ArrayNIJK<Simd<float, 4>, size_t>(
		"curr_v", {numLines[0], numLines[1], numVectors}
	);
	//ParaExec::firstTouch(*m_curr_v, Op->m_perNodePlan);
...
Engine_Tiling::updateVoltageScalarKernel(
	const ArrayNIJK<Simd<float, 4>, size_t>& volt_v,
	const ArrayNIJK<Simd<float, 4>, size_t>& curr_v,
...
)
{
	...
	for (uint32_t vk = first_vk; vk <= last_vk; vk += 1) {
		Simd<float, 4> volt0_ci_cj_ck = volt_v(0, i, j, vk);
		Simd<float, 4> volt1_ci_cj_ck = volt_v(1, i, j, vk);
		Simd<float, 4> volt2_ci_cj_ck = volt_v(2, i, j, vk);

		// 12 (3 x 4) FP32 loads
		Simd<float, 4> curr0_ci_cj_ck = curr_v(0, i, j, vk);
		Simd<float, 4> curr1_ci_cj_ck = curr_v(1, i, j, vk);
		Simd<float, 4> curr2_ci_cj_ck = curr_v(2, i, j, vk);

		// 16 (4 x 4) FP32 loads
		Simd<float, 4> curr0_ci_pj_ck = curr_v(0, i,  pj, vk);
		Simd<float, 4> curr2_ci_pj_ck = curr_v(2, i,  pj, vk);
		Simd<float, 4> curr1_pi_cj_ck = curr_v(1, pi, j,  vk);
		Simd<float, 4> curr2_pi_cj_ck = curr_v(2, pi, j,  vk);

		// 2 misaligned FP32 loads
		Simd<float, 4> curr0_ci_cj_pk;
		curr0_ci_cj_pk[1] = curr0_ci_cj_ck[0];
		curr0_ci_cj_pk[2] = curr0_ci_cj_ck[1];
		curr0_ci_cj_pk[3] = curr0_ci_cj_ck[2];

		Simd<float, 4> curr1_ci_cj_pk;
		...
		// 24 (6 x 4) FP32 loads
		Simd<float, 4> vv0_ci_cj_ck = op.vv_v(0, i, j, vk);
		Simd<float, 4> vv1_ci_cj_ck = op.vv_v(1, i, j, vk);
		Simd<float, 4> vv2_ci_cj_ck = op.vv_v(2, i, j, vk);
		Simd<float, 4> vi0_ci_cj_ck = op.vi_v(0, i, j, vk);
		Simd<float, 4> vi1_ci_cj_ck = op.vi_v(1, i, j, vk);
		Simd<float, 4> vi2_ci_cj_ck = op.vi_v(2, i, j, vk);
...
void Engine_Tiling::updateVoltageRange(
	const ArrayNIJK<Simd<float, 4>, size_t>& volt_v,
	const ArrayNIJK<Simd<float, 4>, size_t>& curr_v,
	const Operator_Tiling& op,
...
)
{
	...
	for (uint32_t vk = first_vk; vk <= last_vk; vk += 1) {
		...
		// 16 (4 x 4) FP32 loads
		Simd<float, 4> volt0_ci_nj_ck = volt_v(0, i,     j + 1, vk);
		Simd<float, 4> volt2_ci_nj_ck = volt_v(2, i,     j + 1, vk);
		Simd<float, 4> volt1_ni_cj_ck = volt_v(1, i + 1, j,     vk);
		Simd<float, 4> volt2_ni_cj_ck = volt_v(2, i + 1, j,     vk);

		// 2 misaligned FP32 loads
		Simd<float, 4> volt0_ci_cj_nk;
		volt0_ci_cj_nk[0] = volt0_ci_cj_ck[1];
		volt0_ci_cj_nk[1] = volt0_ci_cj_ck[2];
		volt0_ci_cj_nk[2] = volt0_ci_cj_ck[3];

		Simd<float, 4> volt1_ci_cj_nk;
		...
		// 24 (6 x 4) FP32 loads
		Simd<float, 4> ii0_ci_cj_ck = op.ii_v(0, i, j, vk);
		Simd<float, 4> ii1_ci_cj_ck = op.ii_v(1, i, j, vk);
		Simd<float, 4> ii2_ci_cj_ck = op.ii_v(2, i, j, vk);
		Simd<float, 4> iv0_ci_cj_ck = op.iv_v(0, i, j, vk);
		Simd<float, 4> iv1_ci_cj_ck = op.iv_v(1, i, j, vk);
		Simd<float, 4> iv2_ci_cj_ck = op.iv_v(2, i, j, vk);
...
void Engine_Tiling::updateCurrentRange(
	const ArrayNIJK<Simd<float, 4>, size_t>& curr_v,
	const ArrayNIJK<Simd<float, 4>, size_t>& volt_v,
	const Operator_Tiling& op,
	const Tiling::Range3D<> range
)
{
	...
	for (uint32_t i = range.first[0]; i <= range.last[0]; i++) {
		for (uint32_t j = range.first[1]; j <= range.last[1]; j++) {
			...
			//if (bodyRange.second == curr_v.extent(2) - 1) {
				if (bodyRange.second - bodyRange.first > veclen - 1) {
