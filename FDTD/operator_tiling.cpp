...
Plan3D Operator_Tiling::makePlan(size_t tileHalfTs)
{
	...
	Plan1D i = computeTrapezoidTiles(gridSize[0], m_tileSize[0], m_tileHalfTs);
	Plan1D j = computeTrapezoidTiles(gridSize[1], m_tileSize[1], m_tileHalfTs);
...
void Operator_Tiling::InitOperator()
{
	...
	m_rawOprArr[OP_TYPE::VV] = new ArrayNIJK<Simd<float, 4>>(
		"vv", {numLines[0], numLines[1], numVectors}
	);
	m_rawOprArr[OP_TYPE::VI] = new ArrayNIJK<Simd<float, 4>>(
		"vi", {numLines[0], numLines[1], numVectors}
	);
	m_rawOprArr[OP_TYPE::IV] = new ArrayNIJK<Simd<float, 4>>(
		"iv", {numLines[0], numLines[1], numVectors}
	);
	m_rawOprArr[OP_TYPE::II] = new ArrayNIJK<Simd<float, 4>>(
		"ii", {numLines[0], numLines[1], numVectors}
	);
	m_index = new ArrayNIJK<uint32_t, uint32_t, 4>(
...
)
{
	...
	ArrayNIJK<Simd<float, 4>>& rawOprArr = *m_rawOprArr[oprType];
	ArrayNIJK<uint32_t, uint32_t, 4>& index = *m_index;
