...
protected:
	...
	ArrayLib::ArrayNIJK<
		// each item is a SIMD vector
		Simd<float, veclen>,

		// Benchmark showed that size_t indices is 1.5x
		// faster than uint32_t indices on 64-bit
		// machines, likely due to reasons related to
		// compiler codegen.
		size_t
	>* m_volt_v;
...
private:
	void updateVoltageRange(const Tiling::Range3D<> range);
	void updateCurrentRange(const Tiling::Range3D<> range);
