...
Tiling::combineTilesTTT(const Plan1D& i, const Plan1D& j, const Plan1D& k)
{
	...
	Plan3D plan({i.numTiles(), j.numTiles(), k.numTiles()}, 8);
...
Tiling::combineTilesTTP(const Plan1D& i, const Plan1D& j, const Plan1D& k)
{
	...
	Plan3D plan({i.numTiles(), j.numTiles(), k.numTiles()}, 4);
	size_t globalSubtileId = 0;
...
std::vector<Plan3D>
Tiling::divideNumaWorkload(Plan3D& plan, size_t numSocket)
{
	// split simulation domain across X axis
	size_t tilesPerSocket = plan.numTiles()[0] / numSocket;

	std::vector<Range1D<size_t>> numaRangeX;
	numaRangeX.push_back(
		Range1D<size_t>{0, tilesPerSocket - 1}
		//Range1D<size_t>(0, 2)  // underallocate node 0 to test load imbalance
	);

	for (size_t i = 1; i < numSocket; i++)
	{
		size_t first = numaRangeX[i - 1].last + 1;
		size_t last = std::min(first + tilesPerSocket, plan.numTiles()[0]);
		//assert(numSocket == 2);
		//size_t last = plan.numTiles()[0];

		numaRangeX.push_back(
			Range1D<size_t>{first, last}
		);
	}

	for (auto& range : numaRangeX)
		printf("%zu - %zu\n", range.first, range.last);

	// create per-socket plan
	std::vector<Plan3D> numaPlan;
	for (size_t i = 0; i < numSocket; i++)
	{
		numaPlan.push_back(
			Plan3D(plan.numTiles(), plan.size())
		);
	}

	for (size_t stage = 0; stage < plan.size(); stage++)
	{
		TileList3D& tileList = plan[stage];

		for (size_t tileId = 0; tileId < tileList.size(); tileId++)
		{
			Tile3D& tile = tileList[tileId];

			bool addedTile = false;
			for (size_t numaId = 0; numaId < numaRangeX.size(); numaId++)
			{
				const Range1D<size_t>& range = numaRangeX[numaId];

				if (tile.id()[0] >= range.first && tile.id()[0] <= range.last)
				{
					numaPlan[numaId][stage].push_back(tile);
					addedTile = true;
					break;
				}
			}
			...
			if (!addedTile)
				throw std::runtime_error(
					"unable to find a matching numaPlan for a tile"
				);
		}
	}
